...
export function HomeDetailsClient({ home }: { home: HomeData }) {
  ...
  const occupancyPercent = (home.maxCapacity ?? 0) > 0 ? Math.round(((home.residentCount ?? 0) / (home.maxCapacity ?? 0)) * 100) : 0

  return (
    <div className="h-full flex flex-col pb-10">
      <div className="flex-shrink-0 mt-2">
        <Link href="/admin/homes" className="inline-flex items-center gap-1 text-sm text-gray-600 hover:text-gray-900">
                  ...
                  <div className="md:col-span-2 rounded-lg border border-gray-200 bg-gray-50/70 p-3">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        defaultChecked={Boolean(home.useCustomNotificationEmail)}
                ...
                <div className="divide-y divide-gray-100">
                  ...
                  <div className="py-3 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-700">{new Date(home.user.createdAt).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                    </div>
                  </div>
                </div>
                    ...
                    <Link key={event.id} href={`/admin/events/${event.id}/edit`} className="block p-4 hover:bg-gray-50/50 transition-colors">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-1 flex items-center gap-1"><Clock className="w-3 h-3" />{new Date(event.startDateTime).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</div>
                        </div>
                      ...
                      <div className="text-xs text-gray-400 mt-1">{event._count.attendances} attendees</div>
                    </Link>
                  ))}
                </div>
              ) : (
