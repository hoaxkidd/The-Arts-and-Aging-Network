...
async function notifyAdminsFacilitatorSummary(requestId: string, summaryText: string) {
  ...
  await db.notification.createMany({
    data: admins.map((admin) => ({
      ...
      message: summaryText,
      link: `/admin/booking-requests/${requestId}`
    }))
  })
}
...
async function autoFinalizeEventRequest(requestId: string, reviewerUserId: string) {
  ...
  await db.notification.create({
    data: {
      ...
      message: `Your booking request has been approved automatically after facilitator confirmations.`,
...
export async function requestExistingEvent(
  ...
  try {
    ...
    if (event.status !== 'PUBLISHED') return { error: "Booking is not available for requests" }
    ...
    if (existingRequest) {
      return { error: "You have already requested this booking" }
    }
    ...
    for (const admin of admins) {
      await db.notification.create({
        data: {
          ...
          link: `/admin/booking-requests/${request.id}`
        }
      })
    }
    ...
    logger.serverAction("Failed to request booking", error)
    const msg = error instanceof Error ? error.message : "Unknown error"
    return { error: process.env.NODE_ENV === "development" ? `Failed to submit booking request: ${msg}` : "Failed to submit booking request" }
  }
}

// Submit booking sign-up form and create booking request (when event requires a form template)
export async function submitEventSignUpForm(
  ...
  try {
    ...
    if (!event.requiredFormTemplateId || !event.requiredFormTemplate) {
      return { error: "This booking does not require a sign-up form" }
    }
    ...
    if (existingRequest) return { error: "You have already requested this booking" }
    ...
    logger.serverAction("Submit booking sign-up form error", error)
    const msg = error instanceof Error ? error.message : "Unknown error"
    return { error: process.env.NODE_ENV === "development" ? `Failed to submit booking request: ${msg}` : "Failed to submit booking request" }
  }
}
...
export async function createCustomEventRequest(data: {
  ...
  try {
    ...
    if (data.formTemplateId && data.formData) {
      ...
      // Extract booking details from form data if available
      ...
      for (const admin of admins) {
        await db.notification.create({
          data: {
            ...
            type: 'EVENT_REQUEST_SUBMITTED',
            title: 'New Booking Request',
            message: `${home.name} has submitted a booking request using form: "${formTemplate.title}"`,
            link: `/admin/booking-requests/${request.id}`
          }
        })
      }
      ...
      for (const admin of admins) {
        await db.notification.create({
          data: {
            ...
            message: `${home.name} has submitted a custom booking request: "${data.title}"`,
            link: `/admin/booking-requests/${request.id}`
          }
        })
      }
    ...
    logger.serverAction("Failed to create custom booking request", error)
...
export async function updateHomeEventRequest(
  ...
  try {
      ...
      if (admins.length > 0) {
        await db.notification.createMany({
          data: admins.map((admin) => ({
            userId: admin.id,
            type: 'EVENT_REQUEST_SUBMITTED',
            title: 'Booking Request Resubmitted',
    ...
    logger.serverAction("Failed to update booking request", error)
  ...
  try {
    ...
    await db.notification.create({
      data: {
        ...
        message: `You can now edit and resubmit your request "${request.customTitle || 'Booking'}".`,
    ...
    logger.serverAction("Failed to grant booking request edit access", error)
...
export async function approveEventRequest(
  ...
  try {
      ...
      await db.$transaction(async (tx) => {
        ...
        await tx.notification.createMany({
          data: targetUserIds.map((userId) => ({
            ...
            message: `${request.geriatricHome.name} needs facilitator RSVP for "${request.customTitle || request.existingEvent?.title || 'Booking request'}"`,
            link: '/staff/bookings'
          }))
        })
      })
    ...
    if (request.type === 'CREATE_CUSTOM') {
      ...
      // For existing booking requests, link the home to the event
      if (!request.existingEventId) {
        throw new Error('Existing booking ID is required for REQUEST_EXISTING type')
      }
      await db.event.update({
        where: { id: request.existingEventId },
        data: { geriatricHomeId: request.geriatricHomeId }
    ...
    if (resolvedApprovalRecipient.email) {
      ...
      const eventTitle = (request.type === 'CREATE_CUSTOM' ? request.customTitle : request.existingEvent?.title) || 'Booking'
      ...
      const eventLink = `${process.env.NEXTAUTH_URL || ''}/dashboard/my-bookings`
    ...
    for (const member of staff) {
      await db.notification.create({
        data: {
          ...
          message: `A new booking "${eventTitle}" is now available for attendance`,
          link: `/staff/bookings/${approvedEventId}`
        }
      })
    }
    ...
    if (approvedEventIds.length > 0) {
      try {
        ...
        logger.log('Email reminders scheduled for approved booking(s)', { eventIds: approvedEventIds }, 'server-action')
      } catch (reminderError) {
...
export async function respondToFacilitatorRequestRsvp(data: {
  ...
  try {
    ...
    revalidatePath('/staff/bookings')
    revalidatePath(`/admin/booking-requests/${data.requestId}`)
    return { success: true }
  } catch (error) {
...
export async function getHomeEventHistory(homeId?: string) {
  ...
  try {
    ...
    logger.serverAction("Failed to get home booking history", error)
    return { error: "Failed to load booking history" }
  }
}
...
// Staff respond to booking request with availability
