...
import { FileText, Calendar, CheckCircle, Clock, XCircle, Search, Lock } from "lucide-react"
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {isHomeAdmin && (
        <div className="flex-shrink-0 mb-4 bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-blue-700">
            Only forms specifically assigned to Home Administrators are shown here. 
      ...
      <div className="flex-shrink-0 flex items-center gap-2 mb-4 border-b border-gray-200">
        <Link
          href={`/dashboard/forms?tab=browse&view=${view}&sort=${sort}&search=${search}&category=${categoryFilter}&status=${statusFilter}`}
          className={cn(
        ...
        <Link
          href={`/dashboard/forms?tab=submissions&view=${view}&sort=${sort}&search=${search}&category=${categoryFilter}&status=${statusFilter}`}
          className={cn(
            "px-4 py-2 text-sm font-medium border-b-2 transition-colors",
            ...
            {templates.length === 0 ? (
              ...
              >
                {templates.map((template: any) => {
                  ...
                  const accessLabel = template.isPublic ? 'All' : (template.allowedRoles ? template.allowedRoles.split(',').map((r: string) => ROLE_LABELS[r as keyof typeof ROLE_LABELS] || r).join(', ') : 'All')
                  return (
          ...
          <div className="flex-1 min-h-0 overflow-auto">
            {mySubmissions.length === 0 ? (
              <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
                ...
                <Link
                  href="/dashboard/forms?tab=browse"
