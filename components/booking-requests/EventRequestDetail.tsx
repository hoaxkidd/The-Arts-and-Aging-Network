...
import { approveEventRequest, closeFacilitatorRsvpPhase, grantEventRequestEditAccess, rejectEventRequest } from '@/app/actions/booking-requests'
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 px-4 py-3">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
          ...
          <span className="text-gray-500">{isCustom ? 'Custom booking' : 'Existing booking'}</span>
        </div>
      </div>

      {/* Booking details card */}
      <div className={cn(STYLES.card)}>
        <h2 className="text-base font-semibold text-gray-900 border-b border-gray-100 pb-2 mb-4">Booking</h2>
      ...
      {hasWeeklySchedule && (
        <div className={cn(STYLES.card)}>
          <h2 className="text-base font-semibold text-gray-900 border-b border-gray-100 pb-2 mb-3">Weekly Booking Dates</h2>
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2 text-sm text-gray-700">
              <span>{preferredDates.length} weekly bookings</span>
              <span className={cn(
      ...
      <div className="bg-white rounded-lg border border-gray-200 px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        {request.status === 'APPROVED' && request.approvedEvent && (
          <Link href={`/bookings/${request.approvedEvent.id}`} className={cn(STYLES.btn, STYLES.btnPrimary, "inline-flex items-center gap-2")}>
