...
import { approveEventRequest, rejectEventRequest } from '@/app/actions/booking-requests'
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input
            id="search-booking-requests"
            name="searchEventRequests"
            type="search"
            placeholder="Search by booking or home..."
            aria-label="Search booking requests by booking or home"
            value={searchQuery}
          ...
          <table className={STYLES.table}>
            <thead className="bg-gray-50">
              <tr className={STYLES.tableHeadRow}>
                ...
                <th className={STYLES.tableHeader}>Booking / Title</th>
              ...
              {filtered.map((req) => {
                ...
                return (
                  <tr key={req.id} className={STYLES.tableRow}>
                    ...
                    <td className={STYLES.tableCell}>
                      <Link href={`/admin/booking-requests/${req.id}`} className="font-medium text-primary-600 hover:underline">
                        {title}
                      </Link>
                      {req.formSubmission && (
                    ...
                    <td className={cn(STYLES.tableCell, "text-right")}>
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {req.status === 'APPROVED' && req.approvedEvent && (
                          <Link href={`/bookings/${req.approvedEvent.id}`} className="text-gray-500 hover:text-primary-600 hover:bg-gray-100 inline-flex items-center gap-1 px-2 py-1 rounded">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={6} className={cn(STYLES.tableCell, "text-center py-12")}>
                    {searchQuery || filterStatus !== 'ALL' ? 'No requests match your filters.' : 'No booking requests to review.'}
                  </td>
                </tr>
              )}
            </tbody>
