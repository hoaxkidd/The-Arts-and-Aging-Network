...
export function checkInNotOpenMessage(checkInWindowMinutes: number | null | undefined): string {
  const minutes = getCheckInWindowMinutes(checkInWindowMinutes)
  if (minutes === 0) return 'Check-in opens at the booking start time'
  if (minutes < 60) return `Check-in opens ${minutes} minute${minutes === 1 ? '' : 's'} before the booking`
  const hours = Math.floor(minutes / 60)
  const rem = minutes % 60
  if (rem === 0) return `Check-in opens ${hours} hour${hours === 1 ? '' : 's'} before the booking`
  return `Check-in opens ${hours}h ${rem}m before the booking`
}
